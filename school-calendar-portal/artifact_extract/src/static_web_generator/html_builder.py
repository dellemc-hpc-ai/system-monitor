...
function displayAllMonths() {{
  ...
  // Show current school year (containing today) + next school year.
  // School years run Aug-Jul. Data covers 2025-2026 and 2026-2027.
  // Cap at May 2027 (last month with school data).
  let displayStart, displayEnd;
  if (nowMonth >= 7) {{
    // In Aug-Dec: current SY started Aug nowYear, ends May nowYear+1
    // Show Aug nowYear (current) through May nowYear+2 (next SY)
    displayStart = new Date(nowYear, 7, 1);
    displayEnd   = new Date(nowYear + 2, 5, 1);
  }} else {{
    // In Jan-Jul: current SY started Aug nowYear-1, ends May nowYear
    // Show Aug nowYear-1 (current) through May nowYear+1 (next SY)
    displayStart = new Date(nowYear - 1, 7, 1);
    displayEnd   = new Date(nowYear + 1, 5, 1);
  }}
  // Cap at May 2027 (last month with school data)
  const DATA_END = new Date(2027, 5, 1);
  if (displayEnd > DATA_END) {{ displayEnd = DATA_END; }}
